...
from loss_fn.kld_lm import KLDiv
...
class Basic:
    def __init__(self,
        ...
        self.cls_loss_acc_fn = hydra.utils.instantiate(cls_loss_acc_fn,
                                                       dataset, device)
        self.distillation_loss = KLDiv(device)
    ...
    def __call__(
        ...
        losses, accuracies = self.cls_loss_acc_fn(outputs, target,
                                                  target_subclips)
        # custom distillation loss during training
        if train_mode and 'lm_pred' in data:
            lm_pred = data['lm_pred']
            output = outputs['logits/action']
            distillation_loss = self.distillation_loss(output, lm_pred)
            losses['distill'] = distillation_loss
