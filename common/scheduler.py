...
class CosineLR(torch.optim.lr_scheduler.CosineAnnealingLR):
    def __init__(self,
        ...
        super().__init__(optimizer,
                         ...
                         # T_max=500000,  # custominfo
                         **kwargs)

    def get_lr(self, *args, **kwargs):
        if self.last_epoch < self.T_max:
            # print("custominfoLR: here getting LR, maybe 0", self.last_epoch, " ", self.T_max)  # custominfo
            ...
            # print("custominfoLR: here setting LR 0: 0.0000003")  # custominfo
            return [0.0000003 for _ in self.optimizer.param_groups]
...
class Warmup(torch.optim.lr_scheduler._LRScheduler):
    ...
    def get_lr(self):
        ...
        # print("custominfo super getlr params: ", self.last_epoch, self.warmup_iters,
        #       self.init_lr_ratio, self.base_lrs, self.base_scheduler.T_max) #custominfo
        assert self.last_epoch < self.warmup_iters  # since it increments
        return [
            0.0000001 + (el * (self.init_lr_ratio + (1 - self.init_lr_ratio) *
